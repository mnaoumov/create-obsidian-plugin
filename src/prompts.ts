...
import { type Answers, buildDerivedBooleans } from './Answers.ts';
...
import { promptPackageManager } from './features/PackageManager.ts';
import { promptPlatformSupport } from './features/PlatformSupport.ts';
...
export async function promptAnswers(defaults?: Partial<Answers>): Promise<Answers> {
  ...
  // Tooling (not for Demo — use defaults)
  let linter = 'eslint';
...
async function promptMetadata(defaults?: Partial<Answers>): Promise<PluginMetadata> {
  const metadata = await group(
    {
      ...
      pluginName: ({ results }) => text({
        defaultValue: defaults?.pluginName ?? makePluginName(String(results.pluginId ?? '')),
        message: 'Plugin display name',
        placeholder: defaults?.pluginName ?? makePluginName(String(results.pluginId ?? '')),
        validate(value): string | undefined {
          if (!value) {
            return 'Should not be empty';
          }
  ...
  return {
    authorGitHubName: String(metadata.authorGitHubName ?? ''),
    authorName: String(metadata.authorName ?? ''),
    fundingUrl: String(metadata.fundingUrl ?? ''),
    pluginDescription: String(metadata.pluginDescription ?? ''),
    pluginId: String(metadata.pluginId ?? ''),
    pluginName: String(metadata.pluginName ?? ''),
  };
}
